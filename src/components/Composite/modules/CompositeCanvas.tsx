...
const CompositeCanvas = forwardRef<CompositeCanvasRef, CompositeCanvasProps>(
  ({ backgroundImage, onCanvasChange }, ref) => {
          ...
          img.onload = () => {
            ...
            if (child.isDragging || child.isSelected || child.isWarping) {
              ...
              if (isWarping && child.warpPoints) {
                ...
                Object.values(wp).forEach((point) => {
                  ctx.fillRect(
                ...
                if (mode === "normal") {
                  ...
                  ctx.fillRect(
                    child.x - handleSize / 2,
                    child.y - handleSize / 2,
                    handleSize,
                    handleSize
                  );
                  ctx.fillRect(
                    child.x + child.width - handleSize / 2,
                    child.y - handleSize / 2,
                    handleSize,
                    handleSize
                  );
                  ctx.fillRect(
                    child.x - handleSize / 2,
                    child.y + child.height - handleSize / 2,
                    handleSize,
                    handleSize
                  );
                  ctx.fillRect(
    ...
    const addChild = useCallback(
      (
        ...
        img.onload = () => {
          ...
          const x =
            transforms?.x || Math.random() * Math.max(0, canvasWidth - width);
          const y =
            transforms?.y || Math.random() * Math.max(0, canvasHeight - height);
    ...
    const getResizeHandle = (
      ...
      const handles = [
        { type: "nw" as const, x: child.x, y: child.y },
        { type: "ne" as const, x: child.x + child.width, y: child.y },
        { type: "sw" as const, x: child.x, y: child.y + child.height },
        {
          type: "se" as const,
            ...
            if (child.id === dragState.elementId) {
              ...
              const updatedWarpPoints = child.warpPoints
                ? {
                    ...
                    topRight: { x: newX + newWidth, y: newY },
                    bottomLeft: { x: newX, y: newY + newHeight },
                    bottomRight: { x: newX + newWidth, y: newY + newHeight },
                  }
                : {
                    topLeft: { x: newX, y: newY },
                    topRight: { x: newX + newWidth, y: newY },
