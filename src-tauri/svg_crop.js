...
async function cropSvgToA4Tiles(svgString, canvasWidthPt, canvasHeightPt) {
  ...
  try {
    ...
    const [origX, origY, origWidth, origHeight] = viewBoxMatch[1].split(/[\s,]+/).map(parseFloat);
    console.error(`Original SVG viewBox: ${origX}, ${origY}, ${origWidth}, ${origHeight}`);
    ...
    console.error(`A4 size in SVG coords: ${A4_WIDTH_SVG} × ${A4_HEIGHT_SVG}`);
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        // Calculate center position for page label in SVG coordinates
        const centerX = tileX + (A4_WIDTH_SVG / 2);
        const centerY = tileY + (A4_HEIGHT_SVG / 2);
        
        // Grid coordinates (1-based for user friendliness)
        const gridCol = col + 1;
        const gridRow = row + 1;
                
        console.error(`Tile ${col},${row}: Page ${pageNumber} (${gridCol},${gridRow}) SVG viewBox(${tileX}, ${tileY}, ${A4_WIDTH_SVG}, ${A4_HEIGHT_SVG})`);
  ...
  <text x="${centerX}" y="${centerY - 20}" text-anchor="middle" font-family="Arial, sans-serif" font-size="48" font-weight="bold" fill="red" fill-opacity="0.5">
    Page ${pageNumber}
  </text>
  <text x="${centerX}" y="${centerY + 30}" text-anchor="middle" font-family="Arial, sans-serif" font-size="36" font-weight="bold" fill="blue" fill-opacity="0.5">
    (${gridCol},${gridRow})
  </text>
</svg>`;
    ...
    console.error(`Created ${tilesSvgs.length} tiles with page numbers`);
    return tilesSvgs;
...
if (require.main === module) {
  ...
  cropSvgToA4Tiles(svgString, parseFloat(canvasWidthPt), parseFloat(canvasHeightPt))
    .then(tiles => {
